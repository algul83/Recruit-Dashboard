...
주어진 채용 공고(JD)와 합격자들의 자료(이력서·포트폴리오·자소서)를 분석해서,
이 회사가 어떤 인재상을 우대했는지 추출하세요.

다음 JSON 형식으로만 응답:

```json
{
  "공통_패턴": ["합격자들이 공통으로 가진 역량/태도/경험 3~6개"],
  "선호_역량": ["JD 우대사항 외에도 합격자들이 실제로 강했던 부분 3~6개"],
  "선호_도메인": ["선호하는 도메인 경험 (예: 헬스케어, 의료데이터, 클라우드 등) 2~5개"],
  "선호_태도": ["선호하는 태도/문화 fit (예: 자기주도 학습, 협업, 임팩트 중심) 3~6개"],
  "차별_포인트": ["JD에 명시되지 않았지만 합격자가 갖춘 결정적 차별 포인트 2~4개"],
  "인재상_요약": "위 분석을 바탕으로 작성한 8~15줄 자연어 인재상 텍스트. 인재상 관리 페이지에 그대로 붙여넣을 수 있는 형식."
}
```

규칙:
- 한국어로 작성.
- 합격자들이 공통적으로 보이는 패턴 위주로 추출. 한 명에게만 있는 특수 경험은 제외.
- 추측 금지. 자료에 근거한 분석만.
- 응답은 위 JSON 한 객체만. 다른 텍스트 X.
"""
...
def learn_ideal_profile(
    jd_text: str,
    hired_examples: list[dict],  # [{name, documents: {파일명: 텍스트}}]
) -> dict[str, Any]:
    """합격자 자료를 분석해 인재상 텍스트 자동 생성."""
    client = _client()

    examples_section = ""
    for ex in hired_examples:
        examples_section += f"\n\n=== 합격자: {ex['name']} ===\n"
    ...
    user_message = (
        f"# 채용 공고 (JD)\n{jd_text}\n\n"
        f"# 합격자 자료 ({len(hired_examples)}명){examples_section}"
    )
