...
이 회사가 어떤 인재상을 우대했는지 **추상화된 패턴**으로 추출하세요.

## 🔒 개인정보 보호 규칙 (절대 준수)
출력 어디에도 **개인 식별 정보를 절대 포함하지 마세요**:
- ❌ 합격자 본인 이름, 영문명, 이니셜
- ❌ 합격자가 다닌 구체적 회사명 (예: "OO기업", "OO스타트업" 같은 익명 표기로)
- ❌ 합격자가 다닌 구체적 학교명·전공 (예: "주요 대학", "공학 전공" 같은 일반화)
...
```json
{
  "인재상_요약": "JD와 합격자 패턴을 종합한 8~15줄 자연어 인재상 텍스트. 인재상 관리 페이지에 그대로 붙여넣을 수 있는 형식. **모든 개인정보/회사명/학교명/프로젝트명 제외, 역량·태도·경험 유형만 표현.**"
}
```

규칙:
- 한국어로 작성.
- 합격자들이 공통으로 보이는 추상화된 패턴 위주. 한 명에게만 있는 특수 경험은 제외.
- 추측 금지. 자료에 근거한 분석만.
- 응답은 위 JSON 한 객체만. 다른 텍스트 X.
- **다시 강조**: 출력 어디에도 합격자 이름·회사명·학교명·프로젝트명 등장 절대 금지.
"""
